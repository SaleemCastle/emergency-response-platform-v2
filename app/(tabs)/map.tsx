...
import { confirmEmergency, EmergencyResponse } from '../utils/api';
...
const NearbyAlerts: React.FC<NearbyAlertsProps> = ({ emergencies, userLocation, onConfirm }) => {
    ...
    const [confirmedEmergencies, setConfirmedEmergencies] = useState<string[]>([]);

    // Move useEffect before any conditional returns
    useEffect(() => {
        const getConfirmedEmergencies = async () => {
            const confirmed = await SecureStore.getItemAsync('confirmedEmergencies');

            console.log('Confirmed emergencies:', confirmed);
            setConfirmedEmergencies(JSON.parse(confirmed || '[]'));
        };
    ...
    const containerStyle = useAnimatedStyle(() => ({
        ...
        opacity: heightAnimation.value > 0 ? 1 : 0,
    }));

    // Memoized calculations after hooks
    const nearbyEmergencies = useMemo(() => 
    ...
    const toggleMinimize = () => {
        ...
        heightAnimation.value = withSpring(isMinimized ? 300 : 80, {
            damping: 15,
            stiffness: 100,
        });
    };
    ...
    return (
        <View style={styles.alertsContainer}>
            ...
            <Animated.View style={[styles.alertsContent, containerStyle]}>
                <ScrollView style={styles.scrollView}>
                    {!isMinimized && nearbyEmergencies.filter(emergency => !confirmedEmergencies.includes(emergency.id)).map(emergency => (
...
export default function MapScreen() {
    ...
    const handleConfirmEmergency = async (emergencyId: string): Promise<void> => {
        const response = await confirmEmergency(emergencyId);
...
const styles = StyleSheet.create({
    ...
    alertsContent: {
        // Add any necessary styles for the alerts content
    },
}); 